#!/usr/bin/env node
// bin/qgsd-fan-out.test.cjs — Wave 0 test scaffold for FAN-01, FAN-02, FAN-03
...
// mapRiskLevelToCount will be exported from hooks/qgsd-prompt.js once implemented.
...
function guardTest(fn) {
  return function() {
    if (!mapRiskLevelToCount) {
      ...
      assert.fail('mapRiskLevelToCount must be exported from hooks/qgsd-prompt.js');
    }
    return fn();
  };
}
