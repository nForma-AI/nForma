...
@~/.claude/qgsd/references/git-integration.md
</required_reading>
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="qgsd-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
node ~/.claude/qgsd/bin/gsd-tools.cjs phases list --type summaries --raw
...
See `~/.claude/qgsd/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/qgsd/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/qgsd/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/qgsd/templates/summary.md`.
...
```bash
node ~/.claude/qgsd/bin/gsd-tools.cjs roadmap update-plan-progress "${PHASE}"
```
...
```bash
node ~/.claude/qgsd/bin/gsd-tools.cjs commit "" --files .planning/codebase/*.md --amend
```
</step>
