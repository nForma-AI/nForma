...
```bash
PHASE_INFO=$(node ~/.claude/qgsd/bin/gsd-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/qgsd/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/qgsd/bin/gsd-tools.cjs find-phase "${PHASE}" --raw)
```
